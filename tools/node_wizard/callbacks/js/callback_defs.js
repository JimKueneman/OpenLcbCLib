    ...
    'cb-can': {
        ...
        functions: [

            {
                ...
                detail: 'Fires for every CAN frame the library receives, before protocol processing. Useful for blinking an activity LED, logging raw frames in GridConnect format for debugging, or counting traffic. The can_msg contains the 29-bit identifier and 0–8 payload bytes. Do not modify the message — it is shared with the protocol handlers.',
                required: false,
                configField: 'on_rx'
            },

            {
                ...
                detail: 'Fires after the library successfully queues a CAN frame for transmission. Useful for activity LEDs and debug logging. The can_msg contains the frame that was sent.',
                required: false,
                configField: 'on_tx'
            },

            {
                ...
                detail: 'Fires when the library allocates or changes the 12-bit CAN alias for a node. The alias is used in CAN frame identifiers to represent the full 48-bit Node ID. Useful for diagnostic logging — print the alias and Node ID so you can correlate CAN traffic during debugging.',
                required: false,
    ...
    'cb-olcb': {
        ...
        functions: [

            {
                ...
                detail: 'Fires when a remote node replies with Optional Interaction Rejected. This means the remote node received your message but does not implement that protocol. The error_code and rejected_mti identify what was rejected. Useful for logging or gracefully handling unsupported features on the network.',
                required: false,
...

            {
                ...
                detail: 'Fires when a remote node replies with Terminate Due To Error. This is more severe than Optional Interaction Rejected — something went wrong processing your message. Log the error_code for diagnostics. Common causes include sending a message to a node that is resetting or has an internal error.',
                required: false,
...

            {
                ...
                detail: 'Fires when a node finishes CAN alias allocation and is about to enter the running state. Return true to allow the node to start operating, or false to delay startup (the library will call again on the next cycle). Use this to perform post-login initialization: start broadcast time consumers, send initial event queries, or verify that required hardware is ready before the node begins accepting commands.',
                required: false,
    ...
    'cb-events': {
        ...
        functions: [
...

            {
                ...
                detail: 'Fires when an event that this node registered as a consumer is reported on the bus. The index tells you which of your registered consumer events matched. This is the primary way to react to events — toggle a relay, change a signal aspect, update a display. The library has already matched the Event ID to your consumer list.',
                required: false,
...

            {
                ...
                detail: 'Fires during event identification when a producer announces the current state (set, clear, or unknown) of an event this node consumes. Use this to synchronize your node state with the network at startup — for example, set initial LED or relay states based on the reported producer state.',
                required: false,
...

            {
                ...
                detail: 'Fires when a Learn Event message is received on the bus. This is part of the event teaching protocol where a producer sends a Learn message and consumers in "learn mode" capture the Event ID. Use this to store the learned Event ID into your configuration memory so the node will react to that event in the future.',
                required: false,
...

            {
                ...
                detail: 'Fires for every Producer/Consumer Event Report (PCER) message on the bus, regardless of whether this node consumes the event. This is an unfiltered bus-wide notification. Useful for event logging, displays that show all network activity, or nodes that need to react to events they did not pre-register as consumers.',
                required: false,
...

            {
                ...
                detail: 'Same as on_pc_event_report but for events that carry additional payload bytes beyond the 8-byte Event ID. The count indicates how many payload bytes are present. Some protocols embed extra data in event reports for efficiency.',
                required: false,
...

            {
                ...
                detail: 'Fires when a Consumer Range Identified message is received. This indicates a remote node consumes a range of sequential Event IDs rather than individual events. Useful for advanced routing or discovery applications.',
                required: false,
    ...
    'cb-config-mem': {
        ...
        functions: [
...

            {
                ...
                detail: 'Same as the read version but for write operations. Flash storage with erase cycles may need longer. Return 0 for immediate writes (most EEPROM), or (0x80 | N) for delayed completion.',
                required: false,
    ...
    'cb-train': {
        ...
        functions: [
...

            {
                ...
                detail: 'Fires after a throttle sends a Set Speed command and the library has updated the internal train state. The speed_float16 is an IEEE half-precision float encoding both speed and direction. Use this to drive your motor controller or DCC output — convert the float16 to a PWM duty cycle or DCC speed step.',
                required: false,
...

            {
                ...
                detail: 'Fires after a throttle sends a Set Function command. The fn_address is the function number (0 = headlight, 1 = bell, 2 = horn, etc.) and fn_value is the new state (0 = off, non-zero = on/value). Use this to control DCC function outputs, sound triggers, or GPIO pins on your hardware.',
                required: false,
...

            {
                ...
                detail: 'Fires when the train enters an emergency stop or emergency off state. The emergency_type distinguishes between local e-stop (single train), global e-stop (all trains via well-known event), and global emergency off (all power cut). The library has already set speed to zero. Use this to immediately cut motor power, apply brakes, or disable outputs.',
                required: false,
...

            {
                ...
                detail: 'Fires after a throttle successfully takes control. The controller_node_id identifies the new controlling throttle. The heartbeat timer starts automatically — the controller must periodically send commands or respond to heartbeat requests to prove it is still active.',
                required: false,
...

            {
                ...
                detail: 'Fires after the controller voluntarily releases this train. The heartbeat timer stops. The train continues at its current speed until another controller takes over or the node is stopped by other means.',
                required: false,
...

            {
                ...
                detail: 'Fires when a listener (another throttle monitoring this train) is added or removed. Listeners receive copies of speed and function commands sent to this train. This is used for consist operations — multiple trains moving together. Use this to update a consist member display if your hardware has one.',
                required: false,
...

            {
                ...
                detail: 'Fires when the controlling throttle fails to respond within the heartbeat deadline. The library has already set speed to zero and activated e-stop, and has notified all listeners. Use this to apply physical brakes or cut motor power on your hardware. The heartbeat mechanism ensures that if a throttle crashes or loses connection, the train stops safely rather than running away.',
                required: false,
...

            {
                ...
                detail: 'Fires when a Controller Changed Notify message is received. This happens when the current controller is transferring control to a different throttle. Return true to accept the new controller, false to reject. If NULL, the library defaults to accepting.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Query Speeds command. Returns three speed values: set_speed (last commanded), commanded_speed (after any limits), and actual_speed (measured, if available). The status byte contains flags for e-stop state and speed type validity. Use this to update your throttle display with the real train state.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Query Function command. Returns the current state of the requested function. Use this to synchronize your throttle function buttons with the actual train state.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Controller Assign command. If result is 0, you now have control and can send speed/function commands. If non-zero, another controller rejected the takeover and current_controller identifies who has control. Update your throttle UI accordingly.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Controller Query command. Returns the Node ID of the current controller (or zero if no controller is assigned). Use this to check if a train is free before attempting to take control.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Controller Changed Notify command. Indicates whether the controller transfer was accepted (result 0) or rejected.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Listener Attach command. Confirms that your node is now receiving copies of speed and function commands sent to that train. Used when building consists — attach your train as a listener on the lead locomotive.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Listener Detach command. Confirms your node is no longer receiving forwarded commands from that train. Used when breaking up consists.',
                required: false,
...

            {
                ...
                detail: 'Fires for each listener in the response to a Listener Query command. The count is the total number of listeners, index is which one this reply is for, and node_id identifies the listener. May fire multiple times if multiple listeners are attached. Use this to display consist membership.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train responds to your Reserve command. Reserve provides temporary exclusive access to the train (prevents other throttles from taking control). Result 0 means the reservation was granted. Use this before performing multi-step operations that should not be interrupted.',
                required: false,
...

            {
                ...
                detail: 'Fires when a train you are controlling sends a heartbeat request. You must send a command (speed, function, or NOOP) before timeout_seconds expires, or the train will emergency stop. The library sends heartbeat requests when the countdown reaches 50%. Set up a timer or use on_100ms_timer to send periodic NOOP commands to keep the heartbeat alive.',
                required: false,
...

            {
                ...
                detail: 'Fires when a throttle searches for a train address and this node matches. The search_address is the DCC address the throttle is looking for, and flags contain match criteria (exact, force long address, etc.). The library has already sent the response. Use this for logging or to update a display showing which throttle found this train.',
                required: false,
    ...
    'cb-bcast-time': {
        ...
        functions: [
...

            {
                ...
                detail: 'Fires when a Report Time event is received from the clock producer, containing the current hour and minute. The clock_state is already updated. This fires during clock synchronization sequences and whenever the clock producer sends a time correction.',
                required: false,
...

            {
                ...
                detail: 'Fires when a Report Date event is received, containing the current month and day. Use this if your application needs to track the simulated calendar date.',
                required: false,
...

            {
                ...
                detail: 'Fires when a Report Year event is received. Useful for era-specific layouts where the simulated year affects operations.',
                required: false,
...

            {
                ...
                detail: 'Fires when the clock rate changes. The rate is a multiplier — 1.0 means real time, 4.0 means the clock runs 4x faster than real time, 0.5 means half speed. Negative rates run the clock backwards. Use this if your application needs to adjust behavior based on how fast time is passing.',
                required: false,
...

            {
                ...
                detail: 'Fires when the clock producer starts the clock (transitions from stopped to running). Use this to resume time-dependent operations that were paused when the clock stopped.',
                required: false,
...

            {
                ...
                detail: 'Fires when the clock producer stops the clock. Time is frozen at the current value. Use this to pause time-dependent operations — for example, hold lighting states steady until the clock resumes.',
                required: false,
...

            {
                ...
                detail: 'Fires when the clock crosses midnight. The date has already been incremented. Use this to reset daily counters, trigger end-of-day events, or log operating session boundaries.',
                required: false,
