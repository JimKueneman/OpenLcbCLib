...
// enumerator is emitted as a flat `export const NAME = N;` (matching the
// #define style), with the values computed in JS — the C preprocessor
// doesn't expand enum values via `cc -E -dM`.
const ENUM_HEADERS = [
    `${ROOT}/src/openlcb/openlcb_types.h`,
    `${ROOT}/src/openlcb/openlcb_application_dcc_detector.h`,
    `${ROOT}/src/openlcb/protocol_stream_handler.h`,
    `${ROOT}/src/openlcb/protocol_config_mem_stream_handler.h`,
];
...
// ---------------------------------------------------------------------------
// 4b. Scan headers for `typedef enum { ... } name_t;` blocks.  Compute each
//     enumerator's integer value in JS (C semantics: starts at 0, increments
//     by 1 for unspecified, `= expr` overrides).  Returns:
//       enumerators: Map<NAME, { kind, value }>   — flat enumerator map
//       enumGroups:  Map<typeName, NAME[]>         — for grouping in JS land
// ---------------------------------------------------------------------------
...
function scanEnums() {
    ...
    const blockRe = /typedef\s+enum\s*\{([\s\S]*?)\}\s*([A-Za-z_][A-Za-z0-9_]*)\s*;/g;
    for (const path of ENUM_HEADERS) {
        const src = stripComments(readFileSync(path, 'utf8'));
        let bm;
        while ((bm = blockRe.exec(src)) !== null) {
            const [, body, typeName] = bm;
            const members = [];
            let runningValue = -1n;  // first unspecified gets 0
            const items = body.split(',').map((s) => s.trim()).filter((s) => s.length);
            for (const item of items) {
                const eq = item.indexOf('=');
                let name, exprText;
                if (eq === -1) { name = item.trim(); exprText = null; }
                else { name = item.slice(0, eq).trim(); exprText = item.slice(eq + 1).trim(); }
                if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)) continue;
                let value;
                if (exprText === null) {
                    value = runningValue + 1n;
                } else {
                    // Resolve references to earlier enumerators in this scan.
                    const resolved = new Map();
                    for (const [n, v] of enumerators) {
                        resolved.set(n, v);
                    }
                    try {
                        const r = evalExpr(exprText, resolved);
                        value = typeof r.value === 'bigint' ? r.value : BigInt(r.value);
                    } catch (e) {
                        // Fall through to running+1; rare with these headers.
                        value = runningValue + 1n;
                    }
                }
                runningValue = value;
                enumerators.set(name, toBigIntOrNumber(value));
                members.push(name);
            }
            if (members.length > 0) groups.set(typeName, members);
        }
    }
    return { enumerators, groups };
}
...
function emit(resolved, skipped, enumGroups) {
    ...
    if (enumGroups && enumGroups.size > 0) {
        lines.push('');
        lines.push('// Enum groupings — same values as the flat exports above, packaged');
        lines.push('// for convenience.  C type names map to JS object names via the');
        lines.push('// transformation: strip leading `_`, drop trailing `_enum`, then');
        lines.push('// PascalCase the snake_case parts.');
        for (const [typeName, members] of [...enumGroups.entries()].sort()) {
            lines.push(`export const ${typeName} = Object.freeze({`);
            for (const m of members) lines.push(`    ${m}: ${m},`);
            lines.push(`});`);
        }
    }
...
function main() {
    ...
    // Merge in enum members + groups.  Enum names take precedence on collision
    // (vanishingly rare — would only happen if a #define and an enumerator
    // shared a name, which is ill-formed C anyway).
    const { enumerators, groups: enumGroups } = scanEnums();
    for (const [n, v] of enumerators) resolved.set(n, v);
    mkdirSync(dirname(OUT_PATH), { recursive: true });
    writeFileSync(OUT_PATH, emit(resolved, skipped, enumGroups));
    console.log(`Wrote ${OUT_PATH}  (${resolved.size} constants, ${enumGroups.size} enum group(s), ${skipped.size} skipped)`);
}
