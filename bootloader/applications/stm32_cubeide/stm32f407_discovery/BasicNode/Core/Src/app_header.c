 ...
 *          linker section, which @c STM32F407VGTX_APPLICATION.ld forces to
 *          address @c APP_HEADER_ADDRESS = 0x08008200
 *          (= @c APP_FLASH_START + @c APP_HEADER_OFFSET).
 *
 ...
 *          CRC validation at boot time.  This matches the TI reference
 *          bootloader's development configuration.
 *
 ...
 *
 * Placed by the linker at 0x08008200.  All checksum fields are zero;
 * a post-link tool must patch them before production programming.
 * ------------------------------------------------------------------------- */

/**
 * @brief Application image header placed at @c APP_HEADER_ADDRESS (0x08008200).
 *
