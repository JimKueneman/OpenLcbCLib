...
const MAIN_NODES = [
  ...
  },
  {
    ...
    h:        L.nodeH,
    desc:     'Top-level entry point called by main(). Calls is_bootloader_requested() first to determine why the bootloader was entered, then passes the result to initialize_hardware(request). Finally calls Bootloader_init(). If Bootloader_init() returns false (the normal case), enters an infinite while(true) loop calling Bootloader_loop() on every iteration.',
    file:     'bootloader.c',
    line:     169,
  },
  {
