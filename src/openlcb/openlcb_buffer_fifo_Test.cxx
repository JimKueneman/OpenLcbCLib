...
// ============================================================================
// push_to_head Tests — Sibling Dispatch Support
// ============================================================================

/**
 * @brief Test push_to_head with a single message
 *
 * Verifies:
 * - push_to_head returns the message pointer on success
 * - Pop returns the same message
 * - FIFO is empty after pop
 */
TEST(OpenLcbBufferFIFO, push_to_head_single_message)
{
    OpenLcbBufferStore_initialize();
    OpenLcbBufferFifo_initialize();
    ...
    openlcb_msg_t *result = OpenLcbBufferFifo_push_to_head(msg);
    EXPECT_EQ(result, msg);
    EXPECT_EQ(OpenLcbBufferFifo_get_allocated_count(), 1);
    EXPECT_FALSE(OpenLcbBufferFifo_is_empty());
...
/**
 * @brief Test push_to_head is popped before normal push
 *
 * Verifies:
 * - Message pushed via push_to_head is returned first by pop
 * - Message pushed via normal push is returned second
 * - FIFO ordering: head-pushed messages have priority
 */
TEST(OpenLcbBufferFIFO, push_to_head_is_next_popped)
{
    OpenLcbBufferStore_initialize();
    OpenLcbBufferFifo_initialize();

    openlcb_msg_t *msgA = OpenLcbBufferStore_allocate_buffer(BASIC);
    openlcb_msg_t *msgB = OpenLcbBufferStore_allocate_buffer(BASIC);
    ASSERT_NE(msgA, nullptr);
    ASSERT_NE(msgB, nullptr);

    // Push A via normal push (tail)
    OpenLcbBufferFifo_push(msgA);

    // Push B via push_to_head (head)
    OpenLcbBufferFifo_push_to_head(msgB);

    EXPECT_EQ(OpenLcbBufferFifo_get_allocated_count(), 2);

    // Pop order: B first (head), then A
    openlcb_msg_t *popped = OpenLcbBufferFifo_pop();
    EXPECT_EQ(popped, msgB);

    popped = OpenLcbBufferFifo_pop();
    EXPECT_EQ(popped, msgA);

    EXPECT_TRUE(OpenLcbBufferFifo_is_empty());

    OpenLcbBufferStore_free_buffer(msgA);
    OpenLcbBufferStore_free_buffer(msgB);
}

/**
 * @brief Test multiple push_to_head calls
 *
 * Verifies:
 * - Multiple head-pushes stack in LIFO order at the front
 * - Pop order: most recent head-push first
 */
TEST(OpenLcbBufferFIFO, push_to_head_multiple)
{
    OpenLcbBufferStore_initialize();
    OpenLcbBufferFifo_initialize();
    ...
    openlcb_msg_t *msgA = OpenLcbBufferStore_allocate_buffer(BASIC);
    openlcb_msg_t *msgB = OpenLcbBufferStore_allocate_buffer(BASIC);
    openlcb_msg_t *msgC = OpenLcbBufferStore_allocate_buffer(BASIC);
    ASSERT_NE(msgA, nullptr);
    ASSERT_NE(msgB, nullptr);
    ASSERT_NE(msgC, nullptr);

    // Push A via normal push
    OpenLcbBufferFifo_push(msgA);

    // Push B via push_to_head
    OpenLcbBufferFifo_push_to_head(msgB);

    // Push C via push_to_head
    OpenLcbBufferFifo_push_to_head(msgC);

    EXPECT_EQ(OpenLcbBufferFifo_get_allocated_count(), 3);

    // Pop order: C, B, A (most recent head-push first)
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgC);
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgB);
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgA);

    EXPECT_TRUE(OpenLcbBufferFifo_is_empty());

    OpenLcbBufferStore_free_buffer(msgA);
    OpenLcbBufferStore_free_buffer(msgB);
    OpenLcbBufferStore_free_buffer(msgC);
}

/**
 * @brief Test push_to_head returns NULL when FIFO is full
 *
 * Verifies:
 * - push_to_head returns NULL when FIFO is at capacity
 * - Existing FIFO contents are unchanged
 */
TEST(OpenLcbBufferFIFO, push_to_head_full_fifo_returns_null)
{
    OpenLcbBufferStore_initialize();
    OpenLcbBufferFifo_initialize();
    ...
    // Fill FIFO to capacity via normal push
    for (int i = 0; i < LEN_MESSAGE_BUFFER; i++)
    {
        OpenLcbBufferFifo_push(&msgs[i]);
    }
    ...
    // push_to_head should fail
    openlcb_msg_t overflow;
    openlcb_msg_t *result = OpenLcbBufferFifo_push_to_head(&overflow);
    EXPECT_EQ(result, nullptr);

    // Count unchanged
    EXPECT_EQ(OpenLcbBufferFifo_get_allocated_count(), LEN_MESSAGE_BUFFER);

    // First pop should still be msgs[0] (original order preserved)
    openlcb_msg_t *popped = OpenLcbBufferFifo_pop();
    EXPECT_EQ(popped, &msgs[0]);
...
/**
 * @brief Test interleaved push and push_to_head
 *
 * Verifies:
 * - Mixed push (tail) and push_to_head operations produce correct pop order
 */
TEST(OpenLcbBufferFIFO, push_to_head_interleaved_with_push)
{
    OpenLcbBufferStore_initialize();
    OpenLcbBufferFifo_initialize();
    ...
    openlcb_msg_t *msgB = OpenLcbBufferStore_allocate_buffer(BASIC);
    openlcb_msg_t *msgC = OpenLcbBufferStore_allocate_buffer(BASIC);
    openlcb_msg_t *msgD = OpenLcbBufferStore_allocate_buffer(BASIC);
    ASSERT_NE(msgA, nullptr);
    ASSERT_NE(msgB, nullptr);
    ASSERT_NE(msgC, nullptr);
    ASSERT_NE(msgD, nullptr);
    ...
    OpenLcbBufferFifo_push_to_head(msgB);
    // Push C (tail)
    OpenLcbBufferFifo_push(msgC);
    // Push D (head)
    OpenLcbBufferFifo_push_to_head(msgD);

    EXPECT_EQ(OpenLcbBufferFifo_get_allocated_count(), 4);

    // State: tail→[D, B, A, C]→head
    // Pop order: D, B, A, C
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgD);
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgB);
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgA);
    EXPECT_EQ(OpenLcbBufferFifo_pop(), msgC);
    ...
    OpenLcbBufferStore_free_buffer(msgB);
    OpenLcbBufferStore_free_buffer(msgC);
    OpenLcbBufferStore_free_buffer(msgD);
}

/**
 * @brief Test push_to_head with tail wraparound past index 0
 *
 * Verifies:
 * - push_to_head correctly wraps tail past index 0
 * - Message is retrieved correctly after wraparound
 */
TEST(OpenLcbBufferFIFO, push_to_head_wraparound)
{
    OpenLcbBufferStore_initialize();
    OpenLcbBufferFifo_initialize();
    ...
    // Fill FIFO partially, then pop to advance tail to a known position
    // Push 5 messages, pop 5 — tail and head are both at index 5
    for (int i = 0; i < 5; i++)
    {
        OpenLcbBufferFifo_push(&msgs[i]);
    }
    ...
    // Now push enough via normal push to fill most of the buffer
    // This moves head forward, wrapping past the end
    for (int i = 0; i < LEN_MESSAGE_BUFFER - 2; i++)
    {
        OpenLcbBufferFifo_push(&msgs[i]);
    }

    // Now push_to_head — this decrements tail, which should wrap from 5 to 4
    openlcb_msg_t head_msg;
    openlcb_msg_t *result = OpenLcbBufferFifo_push_to_head(&head_msg);
    EXPECT_EQ(result, &head_msg);

    // The head-pushed message should be popped first
    openlcb_msg_t *popped = OpenLcbBufferFifo_pop();
    EXPECT_EQ(popped, &head_msg);

    // Then the normally-pushed messages in order
    for (int i = 0; i < LEN_MESSAGE_BUFFER - 2; i++)
    {
        popped = OpenLcbBufferFifo_pop();
        EXPECT_EQ(popped, &msgs[i]);
