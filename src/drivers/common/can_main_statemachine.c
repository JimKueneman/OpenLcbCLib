...
void _handle_run_Init(openlcb_node_t* next_node) {
    ...
    next_node->state.run = RUNSTATE_GENERATE_ALIAS; // Jump over Generate Seed that only is if we have an Alias conflict and have to jump back
...
void _handle_run_generate_seed(openlcb_node_t* next_node) {
    ...
    next_node->state.run = RUNSTATE_GENERATE_ALIAS;
...
void _handle_run_generate_alias(openlcb_node_t* next_node) {
    ...
    next_node->state.run = RUNSTATE_SEND_CHECK_ID_07;
...
void _handle_run_transmit_cid07(openlcb_node_t* next_node) {
    ...
    if (CanTxStatemachine_try_transmit_can_message(&can_msg))
        next_node->state.run = RUNSTATE_SEND_CHECK_ID_06;
...
void _handle_run_transmit_cid06(openlcb_node_t* next_node) {
    ...
    if (CanTxStatemachine_try_transmit_can_message(&can_msg))
        next_node->state.run = RUNSTATE_SEND_CHECK_ID_05;
...
void _handle_run_transmit_cid05(openlcb_node_t* next_node) {
    ...
    if (CanTxStatemachine_try_transmit_can_message(&can_msg))
        next_node->state.run = RUNSTATE_SEND_CHECK_ID_04;
...
void _handle_run_transmit_cid04(openlcb_node_t* next_node) {
    ...
    if (CanTxStatemachine_try_transmit_can_message(&can_msg))
        next_node->state.run = RUNSTATE_WAIT_200ms;
...
void _handle_run_wait_200ms(openlcb_node_t* next_node) {
    ...
    if (next_node->timerticks > 3)
        ...
        next_node->state.run = RUNSTATE_TRANSMIT_RESERVE_ID;
...
void handle_run_transmit_rid(openlcb_node_t* next_node) {
    ...
    if (CanTxStatemachine_try_transmit_can_message(&can_msg))
        next_node->state.run = RUNSTATE_TRANSMIT_ALIAS_MAP_DEFINITION;

}

void _handle_run_transmit_amd(openlcb_node_t* next_node) {

    can_msg_worker.identifier = RESERVED_TOP_BIT | CAN_CONTROL_FRAME_AMD | next_node->alias;
    CanUtilities_copy_node_id_to_payload(&can_msg_worker, next_node->id, 0);


    if (CanTxStatemachine_try_transmit_can_message(&can_msg_worker)) {

        next_node->state.permitted = TRUE;
        next_node->state.run = RUNSTATE_TRANSMIT_INITIALIZATION_COMPLETE;
...
void _handle_run_transmit_initialization_complete(openlcb_node_t* next_node) {
    ...
    openlcb_msg_worker.source_alias = next_node->alias;
    ...
    Utilities_copy_node_id_to_openlcb_payload(&openlcb_msg_worker, next_node->id);

    if (CanTxStatemachine_try_transmit_openlcb_message(&openlcb_msg_worker, &can_msg_worker, 0))
        next_node->state.run = RUNSTATE_TRANSMIT_PRODUCER_EVENTS;
...
void CanMainStateMachine_run() {
    ...
    while (next_node) {
        ...
        if (next_node->state.run)
            break;

        switch (next_node->state.run) {
