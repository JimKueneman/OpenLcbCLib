...
bool _handle_can_frame(can_msg_t *can_msg)
{
    ...
    CanUtilities_copy_can_message(can_msg, &transmitted_can_msg);
...
TEST(CanTxStatemachine, transmit_can_message)
{
    ...
    EXPECT_TRUE(CanTxStatemachine_transmit_can_message(&can_msg));
    EXPECT_TRUE(_handle_can_frame_frame_called);
    EXPECT_TRUE(compare_can_msg(&can_msg, transmitted_can_msg.identifier, transmitted_can_msg.payload_count, transmitted_can_msg.payload));
    _handle_addressed_msg_frame_called = false;
}

TEST(CanTxStatemachine, transmit_openlcb_message)
{
    _initialize();
    ...
    if (openlcb_msg)
    {
        ...
        EXPECT_TRUE(CanTxStatemachine_transmit_openlcb_message(openlcb_msg));
        ...
        OpenLcbUtilities_load_openlcb_message(openlcb_msg, 0xAAA, 0x010203040506, 0xBBB, 0x060504030201, MTI_VERIFY_NODE_ID_GLOBAL, 0);
        EXPECT_FALSE(CanTxStatemachine_transmit_openlcb_message(openlcb_msg));
        EXPECT_FALSE(_handle_unaddressed_msg_frame_called);
        EXPECT_FALSE(_handle_addressed_msg_frame_called);
        EXPECT_FALSE(_handle_datagram_frame_called);
        EXPECT_FALSE(_handle_stream_frame_called);
