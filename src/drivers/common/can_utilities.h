...
extern uint16_t CanUtilties_extract_can_mti_from_can_message(can_msg_t* can_msg);  // caution this return 0 if the can message is a datagram/stream or other message that has a MTI that won't fit in 12 bits
...
extern uint8_t CanUtilities_copy_openlcb_payload_to_can_payload(openlcb_msg_t* openlcb_msg, can_msg_t* can_msg, uint8_t can_bytes_start, uint16_t* openlcb_msg_payload_index);
